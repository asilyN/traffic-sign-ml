#!/usr/bin/env python3
"""
Run inference with the SGD traffic-sign classifier.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model-path",
        type=Path,
        default=Path("models/sgd_classifier.joblib"),
...
def resolve_model_path(root: Path, model_path_arg: Path) -> Path:
    ...
    candidates = [
        ...
        (root / "models" / "sgd_classifier.joblib").resolve(),
        (Path(".").resolve() / model_path_arg).resolve(),
        (Path(".").resolve() / "server" / "app" / "ml" / "dataset" / "models" / "sgd_classifier.joblib").resolve(),
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    ...
    raise FileNotFoundError(
        ...
        f"{searched}\n"
        "Run server/app/ml/scripts/train-sgd-classifier.py first."
    )
